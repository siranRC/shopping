    ...
    //产品的售卖属性值切换高亮
      ...
      async addShopcar() {
        ...
        try {
          ...
          //成功：1.进行路由跳转
           //2. 在路由跳转的时候还需要将产品信息带给下一级路由组件
           //一些简单的数据通skuNum,通过query形式给路由组件传递过去
           //产品信息的数据【比较复杂：skuInfo】,通过会话存储（不持久化，会话结束，数据消失）
           sessionStorage.setItem('SKUINFO',JSON.stringify(this.skuInfo))
          this.$router.push({ name: "addcartsuccess",query:{skuInfo:this.skuInfo,skuNum:this.skuNumber} })

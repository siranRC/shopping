        ...
        >
          ...
          <li class="cart-list-con5">
            ...
            <input
              ...
              class="itxt"
              :value="cart.skuNum"
              @change="handler('change', $event.target.vlue * 1, cart)"
            />
            <a
              href="javascript:void(0)"
              class="plus"
...
export default {
  ...
  methods: {
   ...
   async updataChecked(cart,event){
      //带给服务器的参数isChecked,不是布尔值 应该是0（未选）或1（已选）
    try {
       let isChecked = event.target.checked == true? 1:0
    await this.$store.dispatch('updateCkeckedById',{ skuId:cart.skuId, isChecked })
    //如果修改数据成功，再次发起请求（购物车）
    this.getData()
    } catch (error) {
      alert(error.message)
